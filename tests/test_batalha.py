...
def _analise(nome="A", ret_adv=5.0, ret_neu=10.0, ret_fav=15.0):
    return {"nome": nome, "tipo": "pre", "ret_adv": ret_adv, "ret_neu": ret_neu, "ret_fav": ret_fav}
...
class TestCarteiraMista:
    ...
    def test_chaves_retornadas(self):
        res = self._mix()
        assert set(res) == {
            "ret_adv", "ret_neu", "ret_fav", "risco_std",
            "peso_principal", "peso_liquida",
            "nome_principal", "nome_liquida", "tipo_principal",
        }
    ...
    def test_tipo_principal_preservado(self):
        principal = {"nome": "P", "tipo": "ipca_mais", "ret_adv": 5.0, "ret_neu": 9.0, "ret_fav": 13.0}
        liquida   = _analise("L")
        res = carteira_mista(principal, liquida)

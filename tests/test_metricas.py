...
def _df_ipca_simples():
    """IPCA artificial de 0.5% ao mês desde 2015-01-01 (138 meses = jan/2015–jun/2026)."""
    datas = pd.date_range("2015-01-01", periods=138, freq="MS")
    return pd.DataFrame({"data": datas, "valor": [0.5] * 138, "_is_fallback": False})
...
def _simples(
    ...
    return calcular_posicao_simples(
        titulo, tipo, valor, taxa_pct, dc.isoformat(), dv.isoformat()
    )
...
class TestCalcNtnbNone:
    ...
    def test_pu_invalido_df_vazio_fallback_usa_config(self):
        ...
        df_vazio = pd.DataFrame(
            columns=["nome", "vencimento", "taxa_compra", "taxa_venda"]
        )
...
class TestCalcNtnbFinancas:
    ...
    def test_anos_res_ao_menos_1(self):
        # max(1,...) intencional em calcular_posicao_ntnb — evita "0 anos" na UI
...
class TestCalcSimplesNone:
    def test_compra_igual_vencimento_retorna_none(self):
        ...
        assert (
            calcular_posicao_simples(
                "X", "selic", 10_000.0, 14.75, dc.isoformat(), dc.isoformat()
            )
            is None
        )
    ...
    def test_compra_apos_vencimento_retorna_none(self):
        ...
        assert (
            calcular_posicao_simples(
                "X", "selic", 10_000.0, 14.75, dc.isoformat(), dv.isoformat()
            )
            is None
        )
...
class TestCalcSimplesFinancas:
    ...
    def test_anos_res_ao_menos_1(self):
        # max(1,...) intencional em calcular_posicao_simples — evita "0 anos" na UI

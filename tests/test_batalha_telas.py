...
def _mk(nome="A", tipo="ipca_mais", ret_adv=4.0, ret_neu=10.0, ret_fav=16.0,
        risco_std=2.0, anos_expo=3.0, ret_real=5.0, reinvest=False):
    return {
        "nome": nome, "tipo": tipo,
        "ret_adv": ret_adv, "ret_neu": ret_neu, "ret_fav": ret_fav,
        "risco_std": risco_std, "anos_expo": anos_expo,
        "ret_real": ret_real, "reinvest": reinvest,
    }
...
class TestInsightTexto:
    ...
    def test_pre_reinvest(self):
        w = _mk("Tesouro Prefixado 2029", tipo="pre", reinvest=True,
                ret_neu=12.0, ret_adv=10.0)
        txt = _insight_texto(w, horizonte=5, ipca=5.0, com_ir=False)
        assert "Prefixado" in txt
        assert "Selic" in txt

    def test_pre_expo_zero(self):
        w = _mk("Tesouro Prefixado 2029", tipo="pre", reinvest=False,
                anos_expo=0, ret_neu=14.0, ret_real=6.0)
        txt = _insight_texto(w, horizonte=3, ipca=5.0, com_ir=False)
        assert "14.0" in txt
        assert "MaM" in txt

    def test_pre_expo_positivo(self):
        w = _mk("Tesouro Prefixado 2032", tipo="pre", reinvest=False,
                anos_expo=3.0, ret_neu=13.0, ret_adv=9.0)
        txt = _insight_texto(w, horizonte=2, ipca=5.0, com_ir=False)
        assert "3.0 anos antes" in txt
        assert "adverso" in txt.lower()

    def test_ipca_mais_reinvest(self):
        w = _mk("Tesouro IPCA+ 2032", tipo="ipca_mais", reinvest=True,
                ret_neu=11.0, ret_real=6.0, ret_adv=8.0)
        txt = _insight_texto(w, horizonte=5, ipca=5.0, com_ir=False)
        assert "IPCA+" in txt
        assert "Selic" in txt

    def test_ipca_mais_expo_zero(self):
        w = _mk("Tesouro IPCA+ 2040", tipo="ipca_mais", reinvest=False,
                anos_expo=0, ret_neu=10.0, ret_real=7.0)
        txt = _insight_texto(w, horizonte=14, ipca=5.0, com_ir=False)
        assert "real" in txt.lower()
        assert "7.0" in txt

    def test_ipca_mais_expo_positivo(self):
        w = _mk("Tesouro IPCA+ 2050", tipo="ipca_mais", reinvest=False,
                anos_expo=10.0, ret_neu=10.0, ret_real=6.5, ret_adv=4.0)
        txt = _insight_texto(w, horizonte=5, ipca=5.0, com_ir=False)
        assert "10.0" in txt
        assert "adverso" in txt.lower()
...
class TestAnaliseCached:
    def test_delega_para_analise_batalha(self):
        result = _analise_cached(
            nome="T", tipo="pre", taxa=14.0, anos_total=5.0, anos_saida=5.0,
            ipca=5.0, choque=1.0, com_ir=True, selic=13.0,
        )
    ...
    def test_retorno_ipca_mais(self):
        result = _analise_cached(
            nome="X", tipo="ipca_mais", taxa=7.0, anos_total=10.0, anos_saida=10.0,
            ipca=5.0, choque=1.0, com_ir=False, selic=13.0,
        )

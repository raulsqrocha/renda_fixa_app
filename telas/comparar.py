...
def render():
    ...
    st.caption(
        f"✅ Taxas de referência · carregadas às **{_ts.strftime('%H:%M')}** (atualiza a cada 2h)"
    )
    st.divider()
    ...
    ret_ipca_liq = (
        ret_ipca_bruto * (1 - aliq_ir) if ret_ipca_bruto > 0 else ret_ipca_bruto
    )
    ...
    ret_selic_liq = (
        ret_selic_bruto * (1 - aliq_ir) if ret_selic_bruto > 0 else ret_selic_bruto
    )
    ...
    produtos = [
        {
            ...
            "taxa_str": f"IPCA+ {taxa_ipca_plus:.2f}%",
            "Retorno Bruto": ret_ipca_bruto,
            "Retorno Líquido": ret_ipca_liq,
            "IR": aliq_ir,
            "Isento": False,
        },
        {
            "Produto": "Tesouro Prefixado",
            "taxa_str": f"{taxa_pre:.2f}%",
            "Retorno Bruto": ret_pre_bruto,
            "Retorno Líquido": ret_pre_liq,
            "IR": aliq_ir,
            "Isento": False,
        },
        {
            "Produto": "Tesouro Selic",
            "taxa_str": f"Selic {selic:.2f}%",
            "Retorno Bruto": ret_selic_bruto,
            "Retorno Líquido": ret_selic_liq,
            "IR": aliq_ir,
            "Isento": False,
        },
        {
            "Produto": "CDB",
            "taxa_str": f"{taxa_cdb:.2f}%",
            "Retorno Bruto": ret_cdb_bruto,
            "Retorno Líquido": ret_cdb_liq,
            "IR": aliq_ir,
            "Isento": False,
        },
        {
            "Produto": "LCI",
            "taxa_str": f"{taxa_lci:.2f}% (isento)",
            "Retorno Bruto": ret_lci,
            "Retorno Líquido": ret_lci,
            "IR": 0.0,
            "Isento": True,
        },
        {
    ...
    # -----------------------------------------------------------------------
    st.subheader(
        f"📊  Comparativo — Horizonte de {horizonte} Ano(s) · IR {aliq_ir * 100:.1f}%"
    )
    st.caption(
        f"Capital: {formatar_brl(capital)} · IPCA projetado: {ipca:.1f}% a.a. · Alíquota IR: {aliq_ir * 100:.1f}%"
    )
    ...
    st.info(
        ...
        f"(lucro: {formatar_brl(lucro_melhor)})." + _fgc_aviso,
        icon="💡",
    )
